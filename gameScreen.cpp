...
void show_lose_screen(unsigned char *parlcd_mem_base) {
    ...
    for (i = 0; i < LCD_HEIGHT; i++) {
        for (j = 0; j < LCD_WIDTH/2; j++) { // half the width, because we are writing 2x
...
void show_danger(unsigned char *mem_base, Player *player, FishManager manager){
    uint32_t ledVal;
    if (!manager.playerInDanger(player)){
        ledVal = COLOR_888_BLUE;
    } else{
        ledVal = COLOR_888_ORANGE;
    }
    *(volatile uint32_t*)(mem_base + SPILED_REG_LED_RGB2_o) = ledVal;
}
...
int GameScreen(unsigned char *parlcd_mem_base, unsigned char *mem_base) {
    ...
    parlcd_write_cmd(parlcd_mem_base, 0x2c);
    for (i = 0; i < LCD_HEIGHT; i++) { // half the width, because we are writing 2x
    ...
    while (1) {
        ...
        // First led identifier -> shows whether we have the boost or not
        if (player.getHasBoost()){
            ledVal = COLOR_888_GREEN;
        } else{
            printf("No boost\n");
            ledVal = COLOR_888_RED;
        }

        *(volatile uint32_t*)(mem_base + SPILED_REG_LED_RGB1_o) = ledVal;
        if (stateTime % 20 == 19){
        ...
        show_led_line(mem_base, &player);
        show_danger(mem_base, &player, manager);
