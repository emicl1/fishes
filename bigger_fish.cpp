...
void redraw_main_menu(){
    ...
    for (i=0; i<10; i++)
    {
        draw_char(x, y, fdes, *ch, color, TEXT_SCALE);
    ...
    for (i=0; i<9; i++)
    {
        draw_char(x, y, fdes, *ch1, color, TEXT_SCALE /2);
        x+= (TEXT_SCALE/2)*char_width(fdes, *ch1);
        ch1++;
    }
    x = 120;
    y = 180;
    char str2[]="Show Max Score";
    char *ch2=str2;
    for (i=0; i<16; i++)
    {
        draw_char(x, y, fdes, *ch2, color, TEXT_SCALE /2);
